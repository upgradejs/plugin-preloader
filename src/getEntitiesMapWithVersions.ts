...
function getEntitiesMapWithVersionsSync(entities: Entity[], entitiesMap: Map) {
  ...
  for (const entity of entities) {
    ...
    try {
      ...
      console.log(`Plugin ${name} is not handled yet.`);
    }
  }
  return mapWithVersions;
}

async function getEntitiesMapWithVersionsAsync(entities: Entity[], entitiesMap: Map) {
  const mapWithVersions: MapWithVersions = {};
  for (const entity of entities) {
    const name: Name = Array.isArray(entity) ? entity[0] : entity;
    try {
